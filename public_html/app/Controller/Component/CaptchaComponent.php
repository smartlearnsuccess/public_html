...
class CaptchaComponent extends Component
{
    ...
    function initType()
    {
        if ($this->getType() == 'image') {
            if (!$this->gdInfo()) {
                ...
                } else if (!file_exists($this->fileroot.$this->settings['font'])) {
                    $this->setError("The font file does not exist at the location: " . $this->fileroot.$this->settings['font']);
                    $this->fatalError = true;
                }
            }
        } else {
    ...
    function renderImageCaptcha()
    {
        ...
        if ($this->TTFEnabled()) {
            $textbox = imagettfbbox($font_size, 0, $this->fileroot.$this->settings['font'], $code) or die('Error in imagettfbbox function');
            $x = ($width - $textbox[4]) / 2;
            $y = ($height - $textbox[5]) / 2;
            $y -= 5;
            imagettftext($image, $font_size, 0, $x, $y, $text_color, $this->fileroot.$this->settings['font'], $code) or die('Error in imagettftext function');
        } else if (function_exists("imagestring")) {
            //$font_size = imageloadfont($this->fileroot.$this->settings['font']);
