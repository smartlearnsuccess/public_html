 ...
 */
	protected function _buildItem(&$item, $pos = -1, &$isActive = false) {
		...
		$check = false;
		if (isset($item['url']) && $item['url']['controller']!="") {;
			if ($item['partialMatch']) {
				if($item['type']=="Internal"){
					$check = (strpos(Router::normalize(strtolower($this->here)), Router::normalize(strtolower($item['url'][0]))));
				}
				elseif($item['type']=="Page"){
					if(Router::normalize(strtolower($this->here))=="/"){
						$this->here="Home";
					}
					$check = (strpos('crm'.Router::normalize(strtolower($this->here)), Router::normalize(strtolower($item['url']['controller']))));
				}
				else{
					$check = Router::normalize(strtolower($this->here)) === Router::normalize(strtolower($item['url']['controller']));
				}
				if($check==false)
		...
		} elseif($item['url']['action']=="##"){
			$url = '<a href="'.$item['url']['controller'].'" target="'.$item['target'].'">'.$item['title'].'</a>';
		} else {
