...
class Dashboard extends AppModel
{
    ...
    public function UserWiseGroupByExam($userGroupWiseId)
    {
        ...
        $Exam->bindModel(array('hasAndBelongsToMany' => array('Group' => array('className' => 'Group',
            ...
            'conditions' => array("ExamGroup.group_id" =>$userGroupWiseId)
        ))));
    }
    public function viewdifftype($subject_id, $type, $userGroupWiseId)
    {
        $Question = ClassRegistry::init('Question');
        $quesCount = $Question->find('count', array(
            'joins' => array(
                array('table' => 'diffs', 'type' => 'Inner', 'alias' => 'Diff', 'conditions' => array('Diff.id=Question.diff_id')),
                array('table' => 'question_groups', 'type' => 'LEFT', 'alias' => 'QuestionGroup', 'conditions' => array('Question.id=QuestionGroup.question_id'))),
            'conditions' => array('Question.subject_id' => $subject_id, 'Diff.type' => $type, "QuestionGroup.group_id"=>$userGroupWiseId),
            'group' => 'Question.id'));
        if ($quesCount == NULL)
            $quesCount = 0;
        return $quesCount;
    ...
    public function studentGroups($userGroupWiseId)
    {
        ...
        return $StudentGroup->find('all', array('fields' => array('id', 'group_name'), 'conditions' => array("Group.id" => $userGroupWiseId)));
    }
    ...
    public function recentExamResult($userGroupWiseId)
    {
        ...
        $examList = $Exam->find('all', array(
            'fields' => array('id', 'name', 'start_date', 'end_date', 'passing_percent'),
            'joins' => array(array('table' => 'exam_groups', 'type' => 'INNER', 'alias' => 'ExamGroup', 'conditions' => array('Exam.id=ExamGroup.exam_id'))),
            'conditions' => array('Exam.status' => 'Closed', "ExamGroup.group_id"=>$userGroupWiseId),
            'order' => array('Exam.end_date' => 'desc'),
            'group' => array('id', 'name', 'start_date', 'end_date', 'passing_percent'),
            'limit' => 3));
