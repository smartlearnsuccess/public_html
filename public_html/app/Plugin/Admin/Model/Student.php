...
class Student extends AppModel
{
    ...
    public $validate = array('email' => array('email' => array('rule' => 'email', 'message' => 'Enter a valid email'),
        ...
        'phone' => array(
        	'numeric' => array('rule' => 'numeric', 'required' => true, 'message' => 'Only numbers allowed'),
			'isUnique' => array('rule' => 'isUnique', 'message' => 'This Mobile No. has already been exist! try new one')
		),
    ...
    public function studentRecord($id, $userGroupWiseId)
    {
        $studentArr = $this->find('first', array('joins' => array(array('table' => 'student_groups', 'type' => 'INNER', 'alias' => 'StudentGroup', 'conditions' => array('Student.id=StudentGroup.student_id'))),
            'conditions' => array('Student.id' => $id, 'StudentGroup.group_id' => $userGroupWiseId)
        ));
        return $studentArr;
    }
