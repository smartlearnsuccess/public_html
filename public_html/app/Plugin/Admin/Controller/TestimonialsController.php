...
class TestimonialsController extends AdminAppController
{
    ...
    var $paginate = array('page' => 1, 'order' => array('Testimonial.ordering' => 'ASC'));
    ...
    public function updateorder()
    {
        ...
        foreach (explode(",", $getId) as $k => $id) {
            ++$k;
//            $this->Testimonial->unbindValidation('remove', array('name'), true);
            $Testimonial[] = array('Testimonial' => array('id' => $id, 'ordering' => $k));
            $this->Testimonial->query('UPDATE `testimonials` SET `ordering`=' . $k . ' WHERE `id`=' . $id);
        }
