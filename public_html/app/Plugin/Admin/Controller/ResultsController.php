...
class ResultsController extends AdminAppController
{
    ...
    public function result($id = null)
    {
        try {
            ...
            $examDetails = $this->ExamResult->find('first', array(
                'fields' => array('Exam.name', 'Student.id', 'Student.email', 'ExamResult.percent', 'ExamResult.obtained_marks', 'ExamResult.total_marks', 'Exam.passing_percent', 'Exam.duration','ExamResult.result', 'ExamResult.start_time', 'ExamResult.end_time','ExamResult.test_time','ExamResult.total_test_time', 'Exam.declare_result', 'ExamFeedback.comments'),
                'joins' => array(
                ...
                'conditions' => array('ExamResult.id' => $id, "StudentGroup.group_id"=> $this->userGroupWiseId, 'ExamResult.user_id >' => 0)));
            ...
            $chartRerData = array();
            $chartRerData1 = array();
            $chartRerData = array(array((int)$examDetails['ExamResult']['obtained_marks'], $this->CustomFunction->secondsToHourMinute($examDetails['ExamResult']['test_time'])));
            $chartRerData1 = array(array((int)$examDetails['ExamResult']['total_marks'], $this->CustomFunction->secondsToHourMinute($examDetails['ExamResult']['total_test_time'] * 60)));
