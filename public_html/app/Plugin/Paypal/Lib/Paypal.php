 ...
 **/
	public function __construct($config = array()) {
		...
		$this->errorMessages = array(
			...
			10412 => __('You may have made a second call for the same payment or you may have used the same invoice ID for separate transactions.'),
			10422 => __('Please use a different funcing source.'),
			10445 => __('An error occurred, please retry the transaction.'),
 ...
 **/
    public function getVerifiedStatus($email) {
        ...
        if(in_array($parsed['responseEnvelope']['ack'], array('Success', 'SuccessWithWarning'))) {
            ...
            throw new PaypalException(__d('paypal', 'An error occurred while getting the status of your account.'));
        }
    }
 ...
 **/
	public function storeCreditCard($creditCard) {
		...
		if ($response->code == 200) {
			...
			$message = (isset($responseArray['message'])) ? $responseArray['message'] : __d('paypal', 'There was a problem communicating with the payment gateway') ;
...
/* Check Status of Payment*/
    public function getPaymentDetails($transactionId) {
        try {
            ...
            // Handle the resposne
            if (isset($parsed['TRANSACTIONID']) && $parsed['ACK'] == "Success")  {
