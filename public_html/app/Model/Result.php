...
class Result extends AppModel
{
	...
	public $belongsTo = array('Exam');

	public function userSubject($id)
	{
		$ExamStat = ClassRegistry::init('ExamStat');
		$userSubject = $ExamStat->find('all', array(
			'fields' => array('DISTINCT(Subject.id)', 'Subject.subject_name', 'Subject.ordering'),
			'joins' => array(
			...
			'group' => array('Subject.id', 'Subject.subject_name', 'Subject.ordering'),
	...
	public function userSubjectMarks($id, $subjectId, $sumField, $examId)
	{
		...
		if ($examMaxQuestionArr && $examMaxQuestionArr['ExamMaxquestion']['max_question'] != 0 && $sumField == "marks") {
			...
			$userSubject = $ExamStat->find('first', array(
	...
	public function userSubjectQuestion($id, $subjectId, $examId)
	{
		...
		if ($examMaxQuestionArr && $examMaxQuestionArr['ExamMaxquestion']['max_question'] != 0) {
			...
			$userSubjectQuestion = $ExamStat->find('count', array(
				'conditions' => array('ExamStat.exam_result_id' => $id, 'ExamStat.tsubject_id' => $subjectId)));
		}
	...
	public function userSubjectStatusQuestion($id, $subjectId, $quesStatus)
	{
		$ExamStat = ClassRegistry::init('ExamStat');
		$userSubjectQuestion = $ExamStat->find('count', array(
			'conditions' => array('ExamStat.exam_result_id' => $id, 'ExamStat.tsubject_id' => $subjectId, 'ques_status' => $quesStatus)));
		return $userSubjectQuestion;
	}
	...
	public function userSubjectUnattemptedQuestions($id, $subjectId)
	{
		$ExamStat = ClassRegistry::init('ExamStat');
		$userSubjectQuestion = $ExamStat->find('count', array(
			'conditions' => array('ExamStat.exam_result_id' => $id, 'ExamStat.tsubject_id' => $subjectId, 'answered' => 0)));
		return $userSubjectQuestion;
	}
	...
	public function userSubjectUnattemptedMarks($id, $subjectId)
	{
		...
		$leftQuestionArr = $ExamStat->find('first', array(
	...
	public function userSectionQuestion($id, $exam_id, $type, $studentId)
	{
		...
		$subjectNameArr = $ExamStat->find('all', array(
			...
			'group' => array('Subject.id', 'Subject.subject_name', 'ExamStat.ques_no'),
			'order' => array('ExamStat.ques_no asc')));
		foreach ($subjectNameArr as $value) {
			$userSectionQuestion[$value['Subject']['subject_name']] = $ExamStat->find('all', array(
				'fields' => array('ExamStat.ques_no', 'ExamStat.opened', 'ExamStat.answered', 'ExamStat.review', 'ExamStat.bookmark', 'ExamStat.ques_status'),
